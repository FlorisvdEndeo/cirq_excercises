...
if __name__ == "__main__":
    ...
    circuit.append(
        [cirq.measure(qubit1, key="a"), cirq.measure(qubit2, key="b")]
    )
    ...
    print(
        "  "
        + (
            str(swap_circuit).replace("\n", "\n  ")
            if len(swap_circuit) > 0
    ...
    print(
        "If the swap circuit is correctly implemented, we should obtain the following measurement outcomes:"
    )
    print()
    print("  a={}".format("1" * 20))
    print("  b={}".format("0" * 20))
